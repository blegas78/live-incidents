...
const topics = {
  "incidents.mnpd_crashes": mnpd_crashes_callback,
  "incidents.mnpd_dispatch": mnpd_dispatch_callback,
  "incidents.tdot": tdot_callback,
  "incidents.waze": waze_callback,
}

var kafka = new KafkaMicro("live-incidents", io.sockets, socketHandlers, topics); // using ouster instead
kafkas.push(kafka);
console.log("Added kafka Incidents listener");

function mnpd_crashes_callback ( update ) { 
  const object = JSON.parse(update);

  io.sockets.emit("incidents_mnpd_crashes", object);
}

function mnpd_dispatch_callback ( update ) { 
  const object = JSON.parse(update);

  io.sockets.emit("incidents_mnpd_dispatch", object);
}

function tdot_callback ( update ) { 
  const object = JSON.parse(update);
  console.log(object);

  io.sockets.emit("incidents_tdot", object);
}
function waze_callback ( update ) { 
  const object = JSON.parse(update);

  io.sockets.emit("incidents_waze", object);
}
